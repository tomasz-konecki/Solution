...
class FteBar extends React.PureComponent{
    ...
    putMarkerDivs = range => {
        ...
        for(let i = 0; i < range; i++){
            ...
            divs.push(
                <div key={i} onClick={isChanging ? null : () => this.edit(i)} 
                    onMouseMove={isChanging ? null : () => this.setState({mark: calculateFteBarWidth(valuesArray[i].percentage)})} 
                    onMouseOut={isChanging ? null : () => this.setState({mark: calculateFteBarWidth(this.props.capacityLeft)})}
