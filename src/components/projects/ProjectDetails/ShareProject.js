...
class ShareProject extends Component {
  ...
  shareProject = () => {
    ...
    WebApi.shareProject.post
      .add(this.props.projectId, {
        projectId: this.props.projectId,
        destinationManagersIds: this.state.selectedManagers.map(m => m.id)
      })
      .then(response => {
...
<<<<<<< HEAD
  ...
  onDelete = id => {
    const managers = this.state.managers;
    const managerToDelete = this.state.selectedManagers.filter(
      (value, index, arr) => {
        return value.id === id;
      }
    )[0];
    managers.push(managerToDelete);
    const selectedManagers = this.state.selectedManagers.filter(
      (value, index, arr) => {
        return value.id !== id;
      }
    );
...
>>>>>>> 146008b996af565554fba10452ffd3cddf5d807a
                />
              </div>
            </div>
          </div>
        ...
        <div className="row justify-content-center mx-2">
          <div
            className="col col-sm-5 mt-5 border"
          ...
          >
            ...
            {this.state.selectedManagers.map(manager => {
              return (
                <div
                  key={manager.id}
                  className="row justify-content-end my-2 mr-2"
                >
                  <div style={{ cursor: "default" }}>{manager.fullName}</div>
                  <div
                    className="d-block text-danger mx-2 my-auto"
                    style={{ cursor: "pointer" }}
                    onClick={() => this.onDelete(manager.id)}
                  >
                    X
                  </div>
                </div>
        ...
        <SpinnerButton
          validationResult={
            this.state.selectedManagers != [] && this.state.changed
          }
          transactionEnd={this.state.sharedFinished}
          onClickHandler={this.shareProject}
          isLoading={this.state.isLoading}
