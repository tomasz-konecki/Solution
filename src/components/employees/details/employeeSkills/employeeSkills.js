...
import {
  addNewSkillsToEmployeeACreator,
  addNewSkillsToEmployee
...
const createSkillArchitecture = (skills, limit) => {
  ...
  for (let key in skills) {
    ...
    skillsWithAddData.push({
      skill: skills[key],
      markupWidth: (skills[key].level * 100) / limit,
      color: getRandomColor(),
      width: width,
      startLevel: skills[key].level
    });
  }
...
class EmployeeSkills extends React.Component {
  state = {
    skills: createSkillArchitecture(this.props.skills, this.props.limit),
  ...
  createSpans = arrayElementIndex => {
    ...
    for (let i = 0; i < this.props.limit; i++) {
      spans.push(
        <span
          key={i}
          style={{ width: `${this.state.skills[0].width}%` }}
  ...
  changeWidthOfSpan = (spanIndex, arrayElementIndex) => {
    ...
    skills[arrayElementIndex].markupWidth =
      (100 / this.props.limit) * spanIndex;
  ...
  shouldComponentUpdate(nextState) {
    if (
      nextState.skills === this.state.skills ||
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.changeSkillsStatus) {
      ...
      const newSkills = createSkillArchitecture(
        nextProps.skills,
        nextProps.limit
      );
      ...
      this.setState({
        isLoadingAllSkills: false,
        copiedAllSkills: copiedAllSkills
      });
    }
  }
  copySkills = skills => {
    const copiedAllSkills = [];
    for (let key in skills) copiedAllSkills.push(skills[key]);
    return copiedAllSkills;
  };
  downgradeYears = index => {
    const { skills } = this.state;
    this.manageSkillChange(
      skills[index].skill.yearsOfExperience > minYearsOfExp,
      false,
      index
    );
  };
  ...
  increaseYears = index => {
    const { skills } = this.state;
    this.manageSkillChange(
      skills[index].skill.yearsOfExperience < maxYearsOfExp,
      true,
      index
    );
  };
  manageSkillChange = (shouldChange, shouldAdd, index) => {
    if (shouldChange) {
      ...
      skills[index].skill.yearsOfExperience = shouldAdd
        ? currentYearsOfExp + 1
        : currentYearsOfExp - 1;
  ...
  openSkillsModal = () => {
    ...
    this.setState({
      showAddNewSkillsModal: true,
      isLoadingAllSkills: shouldLoadSkillsAgain
    });
    if (shouldLoadSkillsAgain) getAllSkillsForEmployee(skills);
  };
  ...
  changeSkillsState = (skillName, shouldAdd) => {
    if (shouldAdd) this.addNewSkill(skillName);
    else this.removeSkillFromAdded(skillName);
  };
  ...
  onChange = e => {
    ...
    if (showSkillsToAdd) {
      ...
      for (let key in copiedAllSkills)
        if (
          copiedAllSkills[key].name.toLowerCase().search(valueToSearch) !== -1
        )
  ...
  showSearchBar = () => {
    ...
    const whichArrayToCopy = showSkillsToAdd
      ? [...skillsToAdd]
      : [...copiedAllSkills];
  ...
  render() {
    ...
    const listNameToShow = showSearchBar
      ? "searchedSkills"
      : showSkillsToAdd
        ? "skillsToAdd"
        : "copiedAllSkills";

    const iconForSkills = showSkillsToAdd ? (
      <i className="fa fa-minus" />
    ) : (
      <i className="fa fa-plus" />
    );

    const status = employeeDeleted
      ? "Usunięty"
      : employeeHasAccount
        ? "Aktywny"
        : "Nieaktywny";
        ...
        >
          {isLoadingAllSkills ? (
            ...
            <div className="modal-content-container">
              {showSearchBar ? (
                <header>
                  <h3 className="section-heading">{t("Search")}</h3>
                  <form>
                    ...
                    <input
                      ...
                      type="text"
                      placeholder={
                        showSkillsToAdd ? t("SearchInAdded") : t("SearchInAll")
                      }
                    />
                    <i onClick={this.clearSearchData} className="fa fa-times" />
                  </form>
                </header>
              ) : (
                <header>
                  <h3 className="section-heading">{t("ManageSkills")}</h3>
                  <nav>
                ...
                <ul>
                  {this.state[listNameToShow].map(skill => {
                    return (
                      <li
                        onClick={() =>
                          this.changeSkillsState(skill.name, !showSkillsToAdd)
                        }
                        key={skill.name}
                      >
                        {skill.name}
...
const mapDispatchToProps = dispatch => {
  return {
    getAllSkillsForEmployee: currentAddedSkills =>
      dispatch(getAllSkillsForEmployee(currentAddedSkills)),
    addNewSkillsToEmployeeACreator: (oldSkills, newSkills, employeeId) =>
      dispatch(
        addNewSkillsToEmployeeACreator(oldSkills, newSkills, employeeId)
      ),
    addNewSkillsToEmployee: (status, errors) =>
      dispatch(addNewSkillsToEmployee(status, errors))
  };
};
